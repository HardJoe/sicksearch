...
class VBEPostings:
    ...
    def encode(postings_list):
        ...
        try:
            ...
            postings_list = [
                j - i for i, j in zip(postings_list[:-1], postings_list[1:])
            ]
            postings_list.insert(0, first)
            return VBEPostings.vb_encode(postings_list)
        except IndexError:
            return b""
    ...
    def vb_decode(encoded_bytestream):
        ...
        for _, byte in enumerate(encoded_bytestream):
            if byte < 128:
                n = 128 * n + byte
            else:
                n = 128 * n + byte - 128
...
if __name__ == "__main__":
    ...
    for Postings in [StandardPostings, VBEPostings]:
        ...
        print("test compression.py berhasil")
