...
class InvertedIndexReader(InvertedIndex):
    ...
    def get_postings_list(self, term):
        ...
        sp, num, post_len, tf_len = self.postings_dict[term]
        self.index_file.seek(sp)
        return (
            self.postings_encoding.decode(self.index_file.read(post_len)),
            self.postings_encoding.decode_tf(self.index_file.read(tf_len)),
        )
...
class InvertedIndexWriter(InvertedIndex):
    ...
    def append(self, term, postings_list, tf_list):
        ...
        try:
            sp, num, post_len, tf_len = self.postings_dict[self.terms[-1]]
...
if __name__ == "__main__":
    ...
    with InvertedIndexWriter(
        ...
        ) == [3, 4, 5], "terdapat kesalahan posting"

        assert VBEPostings.decode_tf(
            index.index_file.read(len(VBEPostings.encode_tf([34, 23, 56])))
        ) == [34, 23, 56], "terdapat kesalahan tf"
    print("test index.py berhasil")
